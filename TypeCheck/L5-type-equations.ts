...
import {expComponents} from "./L5-ast";
...
export const makeEquationFromExp = (exp: A.Exp, pool: Pool): Equation[] =>
    ...
    A.isProcExp(exp) ? [makeEquation(inPool(pool, exp), T.makeProcTExp(R.map((vd) => vd.texp, exp.args), inPool(pool, R.last(exp.body))))] :
    // An application must respect the type of its operator
    // Type(Operator) = [T1 * .. * Tn -> Te]
    // Type(Application) = Te
    A.isPairExp(exp) ? [makeEquation(inPool(pool,exp), T.makePairTExp(inPool(pool,exp.left),inPool(pool,exp.right)))] :
    ...
    // The type of a literal is Literal
...
export const canUnify = (eq: Equation): boolean =>
    ...
    (eq.left.paramTEs.length === eq.right.paramTEs.length))? true:
    (T.isPairTExp(eq.left) && T.isPairTExp(eq.right))? true:
         false
...
export const splitEquation = (eq: Equation): Equation[] =>
    (T.isPairTExp(eq.left) && T.isPairTExp(eq.right)) ?
        R.zipWith(makeEquation, [eq.left.left,eq.left.right], [eq.right.left, eq.right.right])  :
    (T.isProcTExp(eq.left) && T.isProcTExp(eq.right)) ?
