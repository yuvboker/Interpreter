...
var iterators = require("composite")
    , map = iterators.map
    , fs = require("fs")
...
assert.deepEqual(L5typeof("(lambda((a : (Pair number number))(b : (Pair number boolean))) :(Pair (Pair number number) (Pair (Pair number number) (Pair number boolean)))(cons a (cons a b)))"),
