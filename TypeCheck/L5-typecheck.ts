...
import {
    isAppExp, isBoolExp, isDefineExp, isEmpty, isIfExp, isLetrecExp, isLetExp, isNumExp, isPairExp,
    isPrimOp, isProcExp, isProgram, isStrExp, isVarRef, parse, unparse,
    AppExp,PairExp, BoolExp, DefineExp, Exp, IfExp, LetrecExp, LetExp, LitExp, NumExp,
    Parsed, PrimOp, ProcExp, Program, SetExp, StrExp, isLitExp
} from "./L5-ast";
...
import {
    ...
    parseTE, unparseTExp,
    BoolTExp, NumTExp, ProcTExp, StrTExp, TExp, LitTExp, makeLitTExp,PairTExp, makePairTExp, isPairTExp
} from "./TExp";
...
const checkEqualType = (te1: TExp | Error, te2: TExp | Error, exp: Exp): true | Error =>
  isError(te1) ? te1 :
  isError(te2) ? te2 :
  (te1.tag!=undefined && te2.tag !=undefined && isPairTExp(te1) && isPairTExp(te2))? true:
  deepEqual(te1, te2) ||
...
export const typeofExps = (exps: Exp[], tenv: TEnv): TExp | Error => {
    if (isDefineExp(first(exps))){
        typeofDefine(first(exps), tenv);
        let def = first(exps),
            newEnv = makeExtendTEnv([def.var.var],[def.var.texp],tenv);
        return typeofExps(rest(exps),newEnv);
    }
...
// a literal has type lit-te
export const typeofLit = (b: LitExp): LitTExp => makeLitTExp();
...
export const typeofPair = (pair: PairExp, tenv: TEnv): TExp | Error => {
    const leftTE = typeofExp(pair.left,tenv);
    const rightTE = typeofExp(pair.right, tenv);
    if(isError(leftTE))
        return leftTE;
    if(isError(rightTE))
        return rightTE;
    return makePairTExp(leftTE , rightTE);
};
...
export const typeofApp = (app: AppExp, tenv: TEnv): TExp | Error => {
    ...
    if (hasNoError(constraints)){
        if(isPrimOp(app.rator)){
            if(app.rator.op == 'car')
                return (typeofExp(app.rands[0],tenv) as PairTExp).left;
            if(app.rator.op == 'cdr')
                return (typeofExp(app.rands[0],tenv) as PairTExp).right;
        }
