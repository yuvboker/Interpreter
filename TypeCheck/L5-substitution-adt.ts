...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error =>
        ...
        isAtomicTExp(e) ? true :
        isPairTExp(e)? (isError(check(e.left)))? check(e.left) :
                        (isError(check(e.right)))? check(e.right):
                            true:
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    isEmptySub(sub) ? te :
    isAtomicTExp(te) ? te :
    isPairTExp(te)? makePairTExp(applySub(sub, te.left),applySub(sub, te.right)):
