...
// TODO: להגדיר אינטרפייס חדש
// לשנות במקומות שקשורים ללמדה להשתמש ב-sharedvaredvardecl
// לזהות לייזי ולהגדיר אותו
...
export type SharedVarDecl = LazyVarDecl | VarDecl;
...
export interface ProcExp {tag: "ProcExp"; args: SharedVarDecl[], body: CExp[]; };
export interface Binding {tag: "Binding"; var: SharedVarDecl; val: CExp; };
...
export const makeProcExp = (args: SharedVarDecl[], body: CExp[]): ProcExp =>
    ({tag: "ProcExp", args: args, body: body});
export const makeBinding = (v: SharedVarDecl, val: CExp): Binding =>
...
const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp( map((decl) => {
        if (decl[1] === 'lazy') {
            return makeLazyVarDecl(decl[0]);
        }
        return makeVarDecl(decl);
    }, sexps[1]), body))
        (map(parseL3CExp, rest(rest(sexps))));

export const parseDecls = (sexps: any[]): SharedVarDecl[] | Error =>
...
// const parseLazyExp = (sexps: any[]):pLazyExp | Error =>
//     sexps.length < 3 ? Error(`Expected (let (<binding>*) <cexp>+) - ${sexps}`) :
//     safeMakeLetExp(parseBindings(sexps[1]),
//                    map(parseL3CExp, sexps.slice(2)));				   
...
const safeMakeBindings = (decls: SharedVarDecl[] | Error, vals: Array<CExp | Error>): Binding[] | Error =>
    isError(decls) ? decls :
