...
import { makeAppExp, makeBoolExp, makeIfExp, makeLitExp, makeNumExp, makeProcExp, makeStrExp,
         makeVarDecl, makeVarRef, SharedVarDecl } from "./L3-ast";
...
// בפונקציה למעלה הוא מזהה שזה פרוצדורה ורוצה להפעיל אותה
// בגרסא הרגילה הוא מעריך את הפרמטרים ואז מפעיל את הפונקצייה
// אנחנו רוצים להעביר את הפרמטרים כמו שהם ולהעריך אותך רק שצריך
const L3applyProcedure = (proc: Value | Error, args: CExp[], env: Env): Value | Error => 
    ...
    isPrimOp(proc) ? applyPrimitive(proc, map((arg) => L3applicativeEval(arg, env) as Value, args)) :
    isClosure(proc) ? applyClosure(proc, map((decPair) => {
        let decl = decPair[0];
        let value = decPair[1];

        if (isLazyVarDecl(decl)) return value as SExp;

        return L3applicativeEval(value, env) as Value;
    }, zip(proc.params, args)), env) :
...
const applyClosure = (proc: Closure, args: Value[], env: Env): Value | Error => {
    let vars = map((v: SharedVarDecl) => v.var, proc.params);
    let body = renameExps(proc.body);
    // let litArgs = map(valueToLitExp, args);
    const decls = zip(args, proc.params);
    //  let litArgs = map(valueToLitExp, args);
    let litArgs = map((decPair) => {		
            
        const value = decPair[0];		
        const decl = decPair[1];		
        
        return isLazyVarDecl(decl) ? value : valueToLitExp(value);		
    }, decls);

    const x = substitute(body, vars, litArgs as CExp[]);
    return evalExps(x, env);
}
...
export const renameExps = (exps: CExp[]): CExp[] => {
    ...
    const replaceProc = (e: ProcExp): ProcExp => {
        const oldArgs = map((arg: SharedVarDecl): string => arg.var, e.args);
